...
import type { OfferRule, ReconciliationRecord, ReportSchedule, ShiftRecord } from '../lib/types';
import { ReportsWorkspace } from '../components/ReportsWorkspace';
import { DeliveryManagement } from '../components/DeliveryManagement';
import { loadReportData, type ReportData } from '../lib/reporting';
...
export function EnterpriseOperationsPage({ section = 'reports', openTransactionId }: { section?: 'reports' | 'reconciliation' | 'deliveries' | 'shifts' | 'offers'; openTransactionId?: string }) {
  ...
  const [reportData, setReportData] = useState<ReportData>({ transactions: [], customers: [], products: [], deliveries: [], users: [] });
  const [isBusy, setIsBusy] = useState(false);
  const [notice, setNotice] = useState('');
  const [autoOpenTransactionId, setAutoOpenTransactionId] = useState(openTransactionId);

  useEffect(() => {
    const handleOpen = (event: Event) => {
      const transactionId = (event as CustomEvent<{ transactionId?: string }>).detail?.transactionId;
      if (transactionId) { setActive('deliveries'); setAutoOpenTransactionId(transactionId); }
    };
    window.addEventListener('jimwas:open-delivery', handleOpen);
    return () => window.removeEventListener('jimwas:open-delivery', handleOpen);
  }, []);

  const refresh = useCallback(async () => {
    setIsBusy(true);
    try {
      const [summary, reconciliationRows, shiftRows, scheduleRows, offerRows] = await Promise.all([
      ...
      setReportData(await loadReportData());
  ...
  return <div className="space-y-6">
    ...
    {active === 'deliveries' && <DeliveryManagement data={reportData} onRefresh={refresh} autoOpenTransactionId={autoOpenTransactionId} />}
